...
1. **Run the four-layer validation** per [statistical-revalidation.md](../skills/validation/statistical-revalidation.md):
   - **Layer 1 — Statistical rigor.** Was the appropriate test selected for the data shape? Were sample sizes adequate? Were resistant statistics applied to skewed metrics? Were effect sizes reported alongside p-values? Was multiple-comparison correction applied? For non-significant claims being reported as "no effect," compute achieved power. **You are the rigor authority for the framework** — the artifact schema accepts Statistics with missing optional fields (effect_size, confidence_interval), but you downgrade Findings citing rigor-incomplete Statistics by one grade and attach a required caveat naming the gap. Schema rejection would throw away an entire stage's analytical work for one missing field; you are the right layer because you operate over Findings (which can be downgraded gracefully) and can calibrate consequence to evidence. The explicit downgrade rules are in [skills/validation/statistical-revalidation.md](../skills/validation/statistical-revalidation.md) — including the "null-result regression / omnibus test" exemptions where missing effect_size or CI is methodologically correct.
