...
| `F_DELIVERY_STOP_DTL_V` | `CCB_PRD.DM` | via `CUSTOMER_SID` — ⚠️ low coverage (~73/623 stores); prefer `F_STOP ROLE='DC'` for delivery activity | RCCB delivery quantities (DELIVERED_QTY, ORDERED_QTY) — limited by historical SID mismatch |
| `F_STOP` | `CCB_PRD.GREEN_MILE_CORE` | via `CUSTOMER_ID → D_CUSTOMER_V → regex(CUSTOMER_DESC)` for store number | GreenMile stop events — ROLE='DC' for delivery (608/623 stores), ROLE LIKE '%MERCH%' for merch (592/623 stores) |
...
- **Delivery Execution**: Use `F_STOP` with `ROLE = 'DC'` for delivery activity — join `F_STOP.CUSTOMER_ID → D_CUSTOMER_V.CUSTOMER_ID` (with `CURRENT_IND='Y'`, `ACCOUNT_GROUP_DESC LIKE '%WALMART%'`, `BUSINESS_TYPE_DESC='DSD'`), then extract store number via `REGEXP_SUBSTR(CUSTOMER_DESC, '#([0-9]+)', 1, 1, 'e', 1)::INTEGER`. This pattern covers **608 of 623 OPD stores (97.6%)** — verified June 2026 via direct Snowflake test. ⚠️ Do NOT join via `F_DELIVERY_STOP_DTL_V.CUSTOMER_SID`: that path only resolves ~73/623 stores (12%) because the delivery table stores historical SIDs that do not match the current-account SIDs in `V_STORE_CUSTOMER`. See verified queries `delivery_volume_vs_ftpr_by_store_week` and `store_ftpr_with_delivery_and_dc` for the correct SQL.
- **Merch Execution**: Join OPD → `V_STORE_CUSTOMER` → `F_STOP` (`CUSTOMER_SID` match) to test whether stores with more merchandising visits or longer merch time have better FTPR. Filter `ROLE LIKE '%MERCH%' AND INSTRUCTIONS LIKE '%MERCH%'` for merch stops. Coverage: 592/623 stores (95%). For higher coverage (608/623), use the same `D_CUSTOMER_V.CUSTOMER_ID` + regex pattern as delivery.
